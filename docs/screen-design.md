...
| カード操作メニュー | カード編集・削除・移動 | メニューボタンから開く。キーボードで操作できる。 |
...
| カード | 白地のコンパクトな面とし、タイトルを主情報として表示する。カード全体を選択可能に見せ、補助操作はホバー時またはメニュー内で表示する。 |
| カード情報 | 今回対象外のラベル、期限、担当者、コメントなどは実データとして実装しない。将来の補助情報領域を想定した余白・配置は持たせない。 |
