...
| リスト | リスト名、操作、並べ替え、カード追加、カード一覧 | 各リストはカードのドロップ先になる。画面の利用可能な高さまで表示する。 |
...
| カード追加 | 「カードを追加」入口、展開後の入力欄と追加・取消 | リスト見出しと並べ替え操作の直下、カード一覧の先頭に表示する。初期状態では入力欄を表示しない。有効な名前で追加後、対象リストの末尾にカードを追加する。 |
...
| リスト内容のあふれ | リストは上部ナビゲーションとボードヘッダーを除く利用可能な画面高まで表示する。カード・入力欄を含む内容がその高さを超える場合のみ、対象リスト内に縦スクロールバーを表示する。 |
| リスト表示領域のあふれ | リスト全体の横幅が画面幅を超える場合のみ、ボードのリスト表示領域に横スクロールバーを表示する。 |
...
| リストの高さとスクロール | リストは利用可能な画面高まで伸ばして表示する。常時スクロールバーを表示せず、カード一覧などの内容が表示範囲を超える場合だけ縦スクロール可能にする。リスト群も画面幅を超える場合だけ横スクロール可能にする。 |
...
| カード追加 | 各リストの上部、見出し・並べ替え操作の直下に「カードを追加」入口を表示する。初期状態では入力欄を常時表示せず、選択時に入力欄と追加・取消ボタンを展開する。 |
